...
# #     """UPDATED: Robust cleaning for master data with new 6-group logic"""
# #     try:
# #         df_clean = df.copy()
...
# #         # STEP 1: Filter by AWD Study participation - UPDATED LOGIC
...
# #         st.info(f"📊 AWD Study Filter: Kept {filtered_count} AWD participants, discarded {initial_count - filtered_count} non-participants")
...
# #         # STEP 3: Payment eligibility - UPDATED for new groups
...
# #             # UPDATED: Payment only for farms with Payment_Eligible = True (A Complied group)
...
# #             st.sidebar.write(f"Payment eligible farms: {master_df['Payment_Eligible'].sum()}")
# #         else:
...
# #         st.write(f"Payment eligible farms (A Complied): {master_df['Payment_Eligible'].sum()}")
...
# #     # Group filter - UPDATED for 6 groups
...
# #     # Status filter - UPDATED for new payment logic
# #     status_options = ['All', 'Compliant Farms Only', 'Non-Compliant Farms Only', 'Payment Eligible Only (A Complied)']
...
# #                 results_df = results_df[results_df['Proportion_Passing'] < 1.0]
# #             elif selected_status == 'Payment Eligible Only (A Complied)':
...
# #                     st.metric("Payment Eligible (A Complied)", payment_eligible_farmers)
...
# #                     st.subheader("💰 Payment Summary (A Complied Group Only)")
# #                     payment_summary = payment_eligible_data.groupby(['Farm_ID', 'Farmer_Name', 'Village', 'Group']).agg({
...
# #     - **Payment** = Eligible Acres × ₹300 (only for A Complied group)
# #     """)

# # # Show UPDATED grouping logic
# # with st.expander("👥 UPDATED: 6-Group System with Compliance Logic"):
# #     st.write("""
# #     **NEW 6-Group Logic Implementation:**
    
# #     **Step 1: AWD Study Filter**
# #     - ✅ **Keep**: `Kharif 25 - AWD Study (Y/N)` = 1
# #     - ❌ **Discard**: `Kharif 25 - AWD Study (Y/N)` = 0 or blank
    
# #     **Step 2: Group Assignment (Hierarchical)**
    
# #     **Group A (Treatment):**
# #     - If `Kharif 25 - AWD Study - Group A - Treatment (Y/N)` = 1, then:
# #       - If `Kharif 25 - AWD Study - Group A - Treatment - complied (Y/N)` = 1 → **"A Complied"** ✅ Payment Eligible
# #       - If `Kharif 25 - AWD Study - Group A - Treatment - Non-complied (Y/N)` = 1 → **"A Non Complied"**
# #       - If neither compliance flag = 1 → **"A Unassigned"**
    
# #     **Group B (Training Only):**
# #     - If `Kharif 25 - AWD Study - Group B -training only (Y/N)` = 1, then:
# #       - If `Kharif 25 - AWD Study - Group B - Complied (Y/N)` = 1 → **"B Complied"**
# #       - If `Kharif 25 - AWD Study - Group B - Non-complied (Y/N)` = 1 → **"B Non Complied"**
# #       - If neither compliance flag = 1 → **"B Unassigned"**
    
# #     **Group C (Control):**
# #     - If `Kharif 25 - AWD Study - Group C - Control (Y/N)` = 1, then:
# #       - If `Kharif 25 - AWD Study - Group C - Complied (Y/N)` = 1 → **"C Complied"**
# #       - If `Kharif 25 - AWD Study - Group C - non-complied (Y/N)` = 1 → **"C Non Complied"**
# #       - If neither compliance flag = 1 → **"C Unassigned"**
    
# #     **Step 3: Payment Eligibility**
# #     - 💰 **Payment Eligible**: ONLY **"A Complied"** group
# #     - ❌ **Not Payment Eligible**: All other groups
    
# #     **Values considered as 1:** 1, 1.0, Y, YES, True, T, X
# #     **Values considered as 0:** 0, 0.0, N, NO, False, F, blank, empty spaces
    
# #     **Final Result: 6 Distinct Groups**
# #     1. **A Complied** (Payment Eligible ✅)
# #     2. **A Non Complied**
# #     3. **B Complied**
# #     4. **B Non Complied**
# #     5. **C Complied**
...
# #     Plus potential edge cases: A/B/C Unassigned, No Group Assigned
# #     """)

# # st.markdown("---")
# # st.markdown("*AWD Compliance Analysis Dashboard v13.0 - NEW 6-Group System with Compliance-Based Payment Logic*")
...
# st.markdown("---")
# st.markdown("*AWD Compliance Analysis Dashboard v14.0 - Enhanced with Unassigned Filtering & Incentive Logic*")
