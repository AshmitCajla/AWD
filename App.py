...
def analyze_farm_compliance(master_df, water_df, farm_pipe_mapping, start_date, end_date):
    ...
    try:
        ...
        for _, farm_data in master_df.iterrows():
            ...
            # Check farm validity (at least 1 pipe with >2 readings)
            farm_valid = False
            ...
            for pipe_id in farm_pipe_codes:
                ...
                # Check for farm validity (at least 1 pipe with >2 readings)
                if len(pipe_data) > 2:
                    farm_valid = True
                else:
            ...
            # REVERTED CALCULATION: Use total assigned pipes as denominator
            if len(farm_pipe_codes) > 0:
                proportion_passing = pipes_passing / len(farm_pipe_codes)  # REVERTED: Back to total assigned pipes
            else:
                proportion_passing = 0  # No assigned pipes → 0% compliance
            ...
            results.append({
                ...
                'Farm_Proportion_Passing': proportion_passing,  # REVERTED: Now based on total assigned pipes
    ...
    try:
        ...
        while current_date <= end_date:
            ...
            for _, farm_data in master_df.iterrows():
                ...
                # Check farm validity for this week (at least 1 pipe with >2 readings)
                farm_valid = False
...
def create_payment_summary(results_df):
    ...
    try:
        ...
        payment_summary = payment_farms[['Village', 'Farm_ID', 'Farmer_Name', 'Group', 'Farm_Valid',
        ...
        if len(date_range) != 2:
            ...
            if results_df is not None and not results_df.empty:
                ...
                if results_df.empty:
                    ...
                    display_df['Farm_Valid'] = display_df['Farm_Valid'].apply(lambda x: "✅" if x else "❌")
                    ...
                    required_columns = [
                        'Village', 'Farm_ID', 'Farmer_Name', 'Group', 'Farm_Valid', 'Total_Incentive_Acres', 
                    ...
                    summary_df = results_df.groupby('Group').agg({
                        'Farm_ID': 'count',
                        'Farm_Valid': 'sum',  # Count of valid farms (True = 1, False = 0)
                    ...
                    # Convert Valid_Farms to integer for better display
                    summary_df['Valid_Farms'] = summary_df['Valid_Farms'].astype(int)
                    ...
                    with st.expander("📅 Weekly Breakdown Analysis", expanded=False):
                        ...
                        if weekly_results is not None and not weekly_results.empty:
                            ...
                            weekly_display['Farm_Valid'] = weekly_display['Farm_Valid'].apply(lambda x: "✅" if x else "❌")
                            ...
                            weekly_display_cols = [
                                ...
                                'Farm_Valid', 'Payment_Eligible', 'Total_Incentive_Acres', 'Assigned_Pipe_IDs', 
                    ...
                    with st.expander("💰 Payment Summary", expanded=False):
                        ...
                        if payment_summary is not None and not payment_summary.empty:
                            ...
                            payment_display['Farm_Valid'] = payment_display['Farm_Valid'].apply(lambda x: "✅" if x else "❌")
...
with st.expander("📏 Compliance Criteria (FIXED)", expanded=False):
    ...
    ### 🏭 **Farm Validity Criterion**:
    - **Valid Farm (✅)**: At least 1 pipe with **>2 readings** in the date range
    - **Invalid Farm (❌)**: No pipes with >2 readings (marked with ❌)
    - Valid farms can still have 0% compliance if no pipes meet criteria
...
with st.expander("👥 Group Assignment Logic", expanded=False):
    ...
    6. **Validate Farms**: Mark farms as valid (✅) or invalid (❌) based on >2 readings criterion
...
with st.expander("📊 Analysis Features & Tables (UPDATED)", expanded=False):
    ...
    - **Farm_Valid**: ✅/❌ indicator for farms with >2 readings criterion ← NEW
    ...
    - **Farm_Valid**: ✅/❌ validity status ← NEW
...
st.markdown("---")
st.markdown("*AWD Compliance Analysis Dashboard v19.0 - **FARM VALIDITY TRACKING** - Added validity status to all tables*")