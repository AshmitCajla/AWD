        ...
        if len(date_range) != 2:
            ...
            if results_df is not None and not results_df.empty:
                ...
                if results_df.empty:
                    ...
                    # FIXED: Show the logic being used
                    st.info("🔧 **UPDATED LOGIC:** Farm compliance = (Pipes passing ÷ Pipes with ≥2 readings) × 100%. Only Valid Farms (≥1 pipe with 2+ readings) included in averages. No gap constraint.")
...
with st.expander("📏 Compliance Criteria (UPDATED)", expanded=False):
    ...
    ### 🆕 **NEW** Valid Farm Definition:
    - **Valid Farm**: Farm with ≥1 pipe having ≥2 readings in the period
    - **Display**: ✅ for valid farms, ❌ for invalid farms
    ...
    ### 🔧 **Key Updates**:
    **1. Compliance Calculation**: Compliant Pipes ÷ **Valid Pipes (≥2 readings)**  
    **2. Valid Farm Filter**: Only farms with ≥1 valid pipe shown as valid  
    **3. Average Calculations**: Group/Village averages use only valid farms
...
st.markdown("---")
st.markdown("*AWD Compliance Analysis Dashboard v19.0 - **VALID FARM LOGIC** - Valid Farms Filter & Compliance Averages Only on Valid Farms*")